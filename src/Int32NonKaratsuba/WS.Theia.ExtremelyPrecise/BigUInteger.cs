...
namespace WS.Theia.ExtremelyPrecise {
	...
	public struct BigUInteger:IComparable, IComparable<BigUInteger>, IEquatable<BigUInteger>, IFormattable {
		...
		public BigUInteger(long value) {
			...
			//TODO:コンテナサイズを変える時に修正する事
			this.Value=new ContainerType[] { (ContainerType)(value&ContainerType.MaxValue),(ContainerType)(value>>BigUInteger.ContainerItemSizeWithBit) };
#endif
		}
		...
		internal BigUInteger(ContainerType[] numerator) {
#if DEBUG&&!DLLDEBUG
		...
		internal BigUInteger(OuterInterfaceType[] numerator) {
			...
			this.Value=(ulong)numerator[0];
#else
			this.Value=new ContainerType[numerator.Length<<1];
			for(var counter = 0;counter<numerator.Length;counter++) {
				this.Value[counter<<1]=(ContainerType)(numerator[counter]&ContainerType.MaxValue);
				this.Value[(counter<<1)+1]=(ContainerType)(numerator[counter]>>BigUInteger.ContainerItemSizeWithBit);
			}
			this.Trim();
#endif
		}
		...
		*/
		...
		public static long ToInt64(BigUInteger value) {
			...
			//TODO:コンテナサイズを変える時に修正する事
			return value.Value.Length>1 ? ((long)value.Value[1]<<BigUInteger.ContainerItemSizeWithBit)|value.Value[0] : value.Value[0];
#endif
		}
		...
		public static ulong ToUInt64(BigUInteger value) {
			...
			//TODO:コンテナサイズを変える時に修正する事
			return value.Value.Length>1 ? ((ulong)value.Value[1]<<BigUInteger.ContainerItemSizeWithBit)|value.Value[0] : value.Value[0];
#endif
		}
		...
		public static BigUInteger operator *(BigUInteger left,BigUInteger right) {
			...
			var result = new BigUInteger() { Value=new ContainerType[left.Value.Length+right.Value.Length] };
			ContainerType carry;
			OuterInterfaceType tempResult;
			int rightCounter;

			for(var leftCounter = left.Value.Length;--leftCounter>=0;) {
				carry=0;
				for(rightCounter=0;rightCounter<right.Value.Length;rightCounter++) {
					tempResult=((OuterInterfaceType)left.Value[leftCounter])*right.Value[rightCounter]+result.Value[leftCounter+rightCounter]+carry;
					result.Value[leftCounter+rightCounter]=(ContainerType)tempResult;
					carry=(ContainerType)(tempResult>>BigUInteger.ContainerItemSizeWithBit);
				}
				if(carry!=0) {
					BigUInteger.MargeAdd(result,leftCounter+right.Value.Length,carry);
				}
			}
