...
namespace WS.Theia.ExtremelyPrecise.Sundbox {
	public class TriangleFunctionTest {
		...
		internal static Rational GetOrder(BigUInteger value) {
			...
			for(var topPart = value.Value;topPart!=0;topPart>>=1) {
				result++;
			}
			return result;
		}
		...
		internal static Rational GetOrder(Rational value) {
			...
			for(var topPart = value.Numerator.Value;topPart!=0;topPart>>=1) {
				result++;
			}
			for(var topPart = value.Denominator.Value;topPart!=0;topPart>>=1) {
				result--;
			}
			return result;
		}
