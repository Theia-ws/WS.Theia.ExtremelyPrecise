	...
	internal class Formatter {
		...
		private StringBuilder IntegerPartFormat() {
			...
			while(!this.IntegerPart.IsZero) {
				...
				if(this.IntegerPart.IsZero) {
					result.Insert(0,cuttedValue.Value.ToString(this.Provider));
				} else {
					result.Insert(0,cuttedValue.Value.ToString("D"+BigUInteger.DecimalChunkOder,this.Provider));
				}
			}
			if(result.Length==0) {
				result.Append("0");
		...
		private string FractionalPartFormat() {
			...
			while(infrator>BigUInteger.DecimalChunkShifter) {
				...
				result.Append(cuttedValue.Value.ToString("D"+BigUInteger.DecimalChunkOder,this.Provider));
				if(this.FractionalPart.IsZero) {
			...
			if(fractionalPartOder!=0) {
				result.Append(this.FractionalPart.Value.ToString("D"+fractionalPartOder,this.Provider));
			}
